...
@Component({
  ...
  selector: "patient-intake-step7",
  templateUrl: "./patient-intake-step7.component.html"
})
export class PatientInTakeStep7Component extends BaseComponent
  ...
  @Output() step7Status: EventEmitter<EditHelperUserAction> = new EventEmitter<
    EditHelperUserAction
  >();
  isEditMode: boolean;
  states: any[];
  ...
  networkStatusSelect: SelectItem[] = [];
   ...
   */
  ngOnInit() {
    this.networkStatusSelect = this.codeService.getNetworkStatusSelect();
  ...
  submitForm() {
    ...
    this.step7Status.emit(userAction);
  }
